...
# Remove the immediate data fetch call from here

tally = None

# Tries to fetch data, if it fails, it will be caught and the error page will be rendered
try:
...
if tally is None:
    layout = html.Div(
        children=[
            html.H1("Error while attempting to fetch data", className="title is-3"),
            html.Br(),
            html.P(
                [
                    "There was an error while trying to fetch the data.  Please try again later or contact the site administrators at ",
                    html.A(
                        "uaf-snap-sys-team@alaska.edu",
                        href="mailto:uaf-snap-sys-team@alaska.edu",
                    ),
                ],
            ),
        ]
    ...
    # Helper functions
    def wrap_in_section(
        content, section_classes="", container_classes="", div_classes=""
    ):
        """
    ...
    <p class="content is-size-5">This chart shows the daily tally for one protection area (<a href="https://fire.ak.blm.gov/content/maps/aicc/Large%20Maps/Alaska_Fire_Management_Zones.pdf">see this map of wildland fire protection areas</a> to see which areas cover which parts of the state).  These data are still being updated and not all years may be present yet.</p><br>
            ...
            html.Div(
                className="graph",
                children=[dcc.Graph(id="tally-zone", config=fig_configs)],
            ),
    ...
    year_dropdown = dcc.Dropdown(
        id="year",
        className="dropdown-selector",
        options=[],  # Placeholder for now, to be filled in the callback
        value=2004,
    )
    ...
    This chart shows the daily tally for each protection area for a given year.  These data are still being updated and not all years may be present yet.
            ...
            html.Div(
                className="graph",
                children=[dcc.Graph(id="tally-year", config=fig_configs)],
            ),
